...
function SplitFlapChar({ char, index, animationKey, skipEntrance, speed, playClick }: SplitFlapCharProps) {
  ...
  const bgColor = isSettled ? "var(--card)" : "color-mix(in oklab, var(--accent) 22%, transparent)"
  const textColor = isSettled ? "var(--foreground)" : "var(--accent)"
  ...
  return (
    ...
    >
      <div className="absolute inset-x-0 top-1/2 h-[1px] bg-foreground/15 pointer-events-none z-10" />
